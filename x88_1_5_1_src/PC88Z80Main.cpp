...
uint8_t CPC88Z80Main::m_abtN88BasicRom[32768];

// N-BASIC ROM

uint8_t CPC88Z80Main::m_abtNBasicRom[32768];

// IE ROM

uint8_t CPC88Z80Main::m_abtIERom[4][8192];

// N80-BASIC ROM

uint8_t CPC88Z80Main::m_abtN80BasicRom[32768];

// N80-BASIC 4TH ROM

uint8_t CPC88Z80Main::m_abtN80Basic4thRom[8192];

// N80V2-BASIC ROM

uint8_t CPC88Z80Main::m_abtN80V2BasicRom[32768];

// N80V2 IE ROM

uint8_t CPC88Z80Main::m_abtN80V2IERom[8192];

// main RAM

uint8_t CPC88Z80Main::m_abtMainRam[65536];

// expansion RAM

uint8_t CPC88Z80Main::m_abtExRam[EX_RAM_COUNT][32768*4];

// fast text VRAM

uint8_t CPC88Z80Main::m_abtFastTVRam[4096];

// graphic VRAM

uint8_t CPC88Z80Main::m_abtGVRam[3][16384];

// graphic VRAM(pixel access)

uint8_t CPC88Z80Main::m_abtGVRamX[16384*8];
...
uint8_t CPC88Z80Main::m_abtKanjiRom[2][131072];

// N80V2 font

// PC-8001mkIISR CG ROM

uint8_t CPC88Z80Main::m_abtFont80SRRom[8192];

// international font ROM

uint8_t CPC88Z80Main::m_abtOptionFontRom[2048];
...
void CPC88Z80Main::Initialize() {
	...
	memset(m_abtExRam, 0, sizeof(m_abtExRam));
	...
	for (i = 0; i < 3; i++) {
		memset(m_abtGVRam[i], 0x00, 16384);
	}
...
uint8_t CPC88Z80Main::ReadMemoryIERom0(int nAddress) {
	return m_abtIERom[0][nAddress & 0x1FFF];
}
...
uint8_t CPC88Z80Main::ReadMemoryIERom1(int nAddress) {
	return m_abtIERom[1][nAddress & 0x1FFF];
}
...
uint8_t CPC88Z80Main::ReadMemoryIERom2(int nAddress) {
	return m_abtIERom[2][nAddress & 0x1FFF];
}
...
uint8_t CPC88Z80Main::ReadMemoryIERom3(int nAddress) {
	return m_abtIERom[3][nAddress & 0x1FFF];
}
...
uint8_t CPC88Z80Main::ReadMemoryExRam(int nAddress) {
	return m_abtExRam[m_nExRamCardNo][m_nExRamBankNo*32768+nAddress];
}
...
void CPC88Z80Main::WriteMemoryExRam(int nAddress, uint8_t btData) {
	m_abtExRam[m_nExRamCardNo][m_nExRamBankNo*32768+nAddress] = btData;
}
...
uint8_t CPC88Z80Main::ReadMemoryGVRam0(int nAddress) {
	return m_abtGVRam[0][nAddress & 0x3FFF];
}
...
void CPC88Z80Main::WriteMemoryGVRam0(int nAddress, uint8_t btData) {
	m_abtGVRam[0][nAddress & 0x3FFF] = btData;
...
uint8_t CPC88Z80Main::ReadMemoryGVRam1(int nAddress) {
	return m_abtGVRam[1][nAddress & 0x3FFF];
}
...
void CPC88Z80Main::WriteMemoryGVRam1(int nAddress, uint8_t btData) {
	m_abtGVRam[1][nAddress & 0x3FFF] = btData;
	UpdateGVRamX(nAddress);
	SetGVRamUpdate(true);
}
...
uint8_t CPC88Z80Main::ReadMemoryGVRam2(int nAddress) {
	return m_abtGVRam[2][nAddress & 0x3FFF];
}
...
void CPC88Z80Main::WriteMemoryGVRam2(int nAddress, uint8_t btData) {
	m_abtGVRam[2][nAddress & 0x3FFF] = btData;
	UpdateGVRamX(nAddress);
	SetGVRamUpdate(true);
}
...
uint8_t CPC88Z80Main::ReadMemoryGVRamEx(int nAddress) {
	...
	m_abtGraphicExPreFetch[0] = m_abtGVRam[0][nAddress2];
	m_abtGraphicExPreFetch[1] = m_abtGVRam[1][nAddress2];
	m_abtGraphicExPreFetch[2] = m_abtGVRam[2][nAddress2];
	return uint8_t(
...
void CPC88Z80Main::WriteMemoryGVRamExALU(int nAddress, uint8_t btData) {
	...
	for (int nPlane = 0; nPlane < 3; nPlane++) {
		...
		case 0:
			m_abtGVRam[nPlane][nAddress2] &= uint8_t(~btData);
			break;
		case 1:
			m_abtGVRam[nPlane][nAddress2] |= btData;
			break;
		case 2:
			m_abtGVRam[nPlane][nAddress2] ^= btData;
			break;
...
void CPC88Z80Main::WriteMemoryGVRamExAll(int nAddress, uint8_t /*btData*/) {
	int nAddress2 = nAddress & 0x3FFF;
	m_abtGVRam[0][nAddress2] = m_abtGraphicExPreFetch[0];
	m_abtGVRam[1][nAddress2] = m_abtGraphicExPreFetch[1];
	m_abtGVRam[2][nAddress2] = m_abtGraphicExPreFetch[2];
	UpdateGVRamX(nAddress);
	SetGVRamUpdate(true);
}
...
void CPC88Z80Main::WriteMemoryGVRamEx1to0(int nAddress, uint8_t /*btData*/) {
	m_abtGVRam[0][nAddress & 0x3FFF] = m_abtGraphicExPreFetch[1];
	UpdateGVRamX(nAddress);
	SetGVRamUpdate(true);
}
...
void CPC88Z80Main::WriteMemoryGVRamEx0to1(int nAddress, uint8_t /*btData*/) {
	m_abtGVRam[1][nAddress & 0x3FFF] = m_abtGraphicExPreFetch[0];
	UpdateGVRamX(nAddress);
	SetGVRamUpdate(true);
}
...
uint8_t CPC88Z80Main::ReadIO_E8(int /*nAddress*/) {
	return m_abtKanjiRom[0][GetKanjiAddress(0)*2+1];
}
...
uint8_t CPC88Z80Main::ReadIO_E9(int /*nAddress*/) {
	return m_abtKanjiRom[0][GetKanjiAddress(0)*2];
}
...
uint8_t CPC88Z80Main::ReadIO_EC(int /*nAddress*/) {
	return m_abtKanjiRom[1][GetKanjiAddress(1)*2+1];
}
...
uint8_t CPC88Z80Main::ReadIO_ED(int /*nAddress*/) {
	return m_abtKanjiRom[1][GetKanjiAddress(1)*2];
}
