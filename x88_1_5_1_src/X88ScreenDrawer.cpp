...
bool CX88ScreenDrawer::Create(CPC88& pc88, bool bInterlace) {
	bool bResult = false;
	m_pPC88 = &pc88;
	m_bInterlace = bInterlace;

	memset(m_abtFonts, 0x00, sizeof(m_abtFonts));
	int nCode;
	...
	memset(m_abBlankFonts, 0, sizeof(m_abBlankFonts));
	...
	if (nMode == MODE_COLOR_640_200) {
		...
		if (!IsInterlace()) {
			do {
				...
				memcpy(pbtBits+LINESTEP, pbtGVRamX, 640);
				pbtGVRamX += 640;
				pbtBits += LINESTEP*2;
			} while (--y != 0);
		} else {
			...
			for (int x = 0; x < 80; x += 2) {
				...
				if ((btAttr & TATTR_REVERSE) == 0) {
					if ((btAttr & TATTR_OVERLINE) != 0) {
						for (int x2 = 0; x2 < 8; x2++) {
							*pbtBits2 = *(pbtBits2+1) =
								*(pbtBits2+LINESTEP) = *(pbtBits2+LINESTEP+1) =
									btColor;
							pbtBits2 += 2;
						}
						pbtFonts++;
